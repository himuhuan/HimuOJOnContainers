<template>
	<transition name="fade">
		<div class="spining-container" v-if="stateRef.loading">
			<n-spin></n-spin>
			<n-text> Loading...</n-text>
		</div>
	</transition>
	<n-alert v-if="stateRef.hasError" type="error" title="加载失败" closable>
			...
			<n-collapse :default-expanded-names="['source-code', 'run-result']">
				...
				>
					<n-alert
						...
						type="error"
					>
						HimuOJ 检测到该提交的代码可能还有试图执行非法代码，已终止评测。
						<br />
				...
				<n-collapse-item title="运行结果" name="run-result">
					<n-alert type="warning" title="注意">
						HimuOJ 官方测试不会限制用户自由下载测试的相关数据与提交源代码,
						部分题目由于发布者的设置可能会限制部分数据的下载，如有疑问请联系题目发布者。
					</n-alert>
