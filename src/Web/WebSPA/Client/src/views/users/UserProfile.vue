		...
		<n-card class="mt-4">
			<n-tabs type="line" animated>
				...
				>
					...
					<n-alert v-else type="error" show-icon title="你不是题目管理员">
...
import { getUserDetail, uploadUserAvatar } from "@/services/usersApi";
...
function doUploadAvatar(event: Event) {
  const input = event.target as HTMLInputElement;
  if (input.files && input.files.length > 0) {
    const file = input.files[0];

    loadingBar.start();
    uploadUserAvatar(props.id, file)
      .then((_) => {
        loadingBar.finish();
        notification.info({
          title: "头像上传成功",
          content: "新头像可能需要重新登录或稍等片刻才能生效。",
          duration: 1000,
        });
      })
      .catch((err) => {
